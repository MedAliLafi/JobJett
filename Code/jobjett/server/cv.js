...
cvRoutes.post('/updateCV', async (req, res) => {
    try {
        const pool = req.pool;
        const { summary, skills, searchable } = req.body;
        ...
        const updateSql = 'UPDATE cv SET Summary = ?, Skills = ?, Searchable = ? WHERE CandidateID = ?';
        const updateValues = [summary, skills, searchable, candidateId];
