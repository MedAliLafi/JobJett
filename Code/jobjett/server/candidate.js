...
function getCandidateInfoById(pool, candidateId) {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT Candidate.FirstName, User.Email, Candidate.LastName, Candidate.DateOfBirth, Candidate.Phone, Candidate.Address, Candidate.State, Candidate.Country, CV.CV_ID FROM Candidate INNER JOIN User ON Candidate.UserID = User.UserID LEFT JOIN CV ON Candidate.CandidateID = CV.CandidateID WHERE Candidate.CandidateID = ?",
      [candidateId],
      (error, results) => {
        if (error) {
          reject(error);
...
function getCandidateIdFromToken(pool, req) {
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT CandidateID FROM candidate WHERE UserID = ?",
      [userID],
      (error, results) => {
        if (error) {
          reject(error);
  ...
  try {
    ...
    registerUser(
      ...
      async (error, result) => {
        ...
        const sql =
          "INSERT INTO Candidate (UserID, FirstName, LastName, DateOfBirth, Phone, Address, State, Country) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
          userID,
          firstName,
          lastName,
        ...
        pool.query(sql, values, async (error, candidateResult) => {
          if (error) {
            ...
            return res.status(500).json({
              error: "An error occurred while registering the candidate.",
            });
          }

          // Insert a new CV record
          const cvSql =
            "INSERT INTO CV (CandidateID, Summary, Skills, Searchable) VALUES (?, ?, ?, ?)";
          ...
          pool.query(cvSql, cvValues, (cvError, cvResult) => {
            if (cvError) {
              ...
              return res
                .status(500)
                .json({ error: "An error occurred while adding the CV." });
            }

            // Update the candidate record with the CV_ID
            const updateSql =
              "UPDATE Candidate SET CV_ID = ? WHERE CandidateID = ?";
            const updateValues = [cvResult.insertId, candidateResult.insertId];

            pool.query(updateSql, updateValues, (updateError, updateResult) => {
              if (updateError) {
                console.error(
                  "Error updating candidate record with CV_ID:",
                  updateError
                );
                return res.status(500).json({
                  error:
                    "An error occurred while updating the candidate record.",
                });
              }
...
candidateRoutes.get("/candidateInfo", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "An error occurred while fetching candidate information",
    });
  }
});
